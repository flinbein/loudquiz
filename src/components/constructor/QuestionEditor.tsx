...
const inputCls =
  "w-full bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg px-3 py-2 text-sm border focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent";
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-lg p-4 border border-slate-200 dark:border-slate-700 space-y-3">
      <div className="flex items-center justify-between">
        <span className="text-slate-500 dark:text-slate-400 text-sm font-medium">Вопрос {index + 1}</span>
        <button
          onClick={onDelete}
      ...
      {/* Text */}
      <div>
        <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Текст вопроса *</label>
        <textarea
          value={question.text}
          onChange={(e) => update({ text: e.target.value })}
          rows={2}
          className={`${inputCls} resize-none ${hasError("text") ? borderError : borderNormal}`}
      ...
      {/* Difficulty */}
      <div>
        <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Сложность *</label>
        <select
          value={question.difficulty}
          onChange={(e) => update({ difficulty: Number(e.target.value) })}
          className={`bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg px-3 py-2 text-sm border focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent ${
            hasError("difficulty") ? borderError : borderNormal
          }`}
        >
          {DIFFICULTIES.map((d) => (
      ...
      {/* Hint */}
      <div>
        <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Подсказка для ведущего</label>
        <input
          type="text"
          value={question.hint ?? ""}
      ...
      {/* AI Answer Check */}
      {showAiCheck && (
        <div className="border border-violet-200 dark:border-indigo-800 rounded-lg overflow-hidden">
          <button
            onClick={() => setCheckOpen((v) => !v)}
            className="w-full flex items-center justify-between px-3 py-2 bg-violet-50 dark:bg-indigo-900/30 hover:bg-violet-100 dark:hover:bg-indigo-900/50 transition-colors text-left"
          >
            <span className="text-xs text-violet-700 dark:text-indigo-300 font-medium">AI Проверка ответов</span>
            <span className="text-violet-500 dark:text-indigo-400 text-xs">{checkOpen ? "▲" : "▼"}</span>
          </button>

          {checkOpen && (
            <div className="p-3 space-y-2 bg-slate-50 dark:bg-slate-900/40">
              ...
              <textarea
                ...
                rows={4}
                placeholder={"Труба\nГитара\nСкрипка\nФлейта"}
                className={`${inputCls} resize-none ${borderNormal}`}
              />
              <button
                onClick={handleCheck}
                disabled={checking || !answersInput.trim()}
                className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg text-sm text-white transition-colors"
              >
              ...
              {checkResult && (
                <div className="space-y-2">
                  {checkResult.groups.map((group) => (
                    <div
                      ...
                      className={`rounded-lg px-3 py-2 text-sm border ${
                        group.accepted
                          ? "bg-emerald-50 dark:bg-green-900/30 border-emerald-200 dark:border-green-700"
                    ...
                    >
                      <div className="flex items-start gap-2">
                        ...
                        <div className="min-w-0">
                          <span className="font-medium text-slate-900 dark:text-white">{group.canonicalAnswer}</span>
                  ...
                  {checkResult.commentary && (
                    <p className="text-xs text-slate-500 dark:text-slate-400 italic pt-1">{checkResult.commentary}</p>
                  )}
                </div>
              )}
            </div>
